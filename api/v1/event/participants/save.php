...
try {
    if ($id) {
        ...
        if (!$checkStmt->fetch()) {
            sendResponse(false, "प्रतिभागी नहीं मिला या आप उसे संपादित करने के लिए अधिकृत नहीं हैं");
        }
        
        $stmt = $pdo->prepare("UPDATE em_participants SET 
            name = ?, phone = ?, city = ?, address = ?, age = ?, gender = ?, category = ?, group_number = ?, notes = ? 
            WHERE id = ? AND event_id = ?");
        $stmt->execute([$name, $phone, $city, $address, $age, $gender, $category, $group_number, $notes, $id, $auth['event_id']]);
        $message = "प्रतिभागी जानकारी सफलतापूर्वक अपडेट की गई";
    } else {
        ...
        $id = $pdo->lastInsertId();
        $message = "प्रतिभागी सफलतापूर्वक जोड़ा गया";
    }
    ...
    sendResponse(true, $message, $participant);
