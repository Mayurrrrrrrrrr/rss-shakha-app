...
foreach ($files as $f) {
    ...
    // Replace CSS: from .sub-capture-container to .sub-footer-band { ... }
    $c = preg_replace('/\.sub-capture-container.*?\.sub-footer-band[^{]*{[^}]*}\s*/s', $cssReplacement . "\n", $c);
    
    // Replace HTML (subhashit.php has <div class="sub-capture-container"> to </div><!-- END capture-area --> approx)
    $pattern = '/<div id="capture-area" class="sub-capture-container">.*?<div class="sub-floral-bottom"[^>]*>.*?<\/div>\s*<\/div>\s*<\/div>/s';
    if (preg_match($pattern, $c)) {
        $c = preg_replace($pattern, $htmlReplacementSubhashit . "\n        </div>\n    </div>", $c);
    } else {
        // subhashit_view.php has a slightly different ending
        $pattern2 = '/<div id="capture-area" class="sub-capture-container">.*?<div class="sub-floral-bottom"[^>]*>.*?<\/div>\s*<\/div>\s*<\/div>/s';
        $c = preg_replace($pattern2, $htmlReplacementSubhashit . "\n        </div>\n    </div>", $c);
    }

    // Replace the JS class names
    $c = str_replace('<div class="sub-shabdarth-row">', '<div class="shabd-row">', $c);
    $c = str_replace('<span class="sub-shabdarth-word">', '<span class="shabd-word">', $c);
    $c = str_replace('<span class="sub-shabdarth-meaning">', '<span class="shabd-word" style="color:#002266; font-weight:400;">', $c);
    $c = str_replace('<span class="sub-shabdarth-dash">—</span>', '—', $c);
    $c = str_replace("backgroundColor: '#FFF9E6'", "backgroundColor: '#1a1100'", $c);

    // Font injection
    if (strpos($c, 'Noto+Serif+Devanagari') === false) {
        $c = str_replace('<style>', "<link href=\"https://fonts.googleapis.com/css2?family=Noto+Serif+Devanagari:wght@400;700;800;900&display=swap\" rel=\"stylesheet\">\n<style>", $c);
    }

    // Remove the old preview JS that forces font size (since the new CSS uses clamp())
    $c = preg_replace('/\/\/ Auto-size sanskrit text.*?sanskritEl\.style\.fontSize = fontSize \+ \'px\';/s', '', $c);
