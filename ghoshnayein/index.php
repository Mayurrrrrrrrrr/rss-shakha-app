...
function getGhoshnaHtml($item) {
    ...
    if (!empty($item['slogan_sanskrit'])) {
        $contentHtml .= '<div class="content-item-sanskrit">' . nl2br(htmlspecialchars($item['sanskrit_text'] ?? $item['slogan_sanskrit'])) . '</div>';
    }
...
<?php else: ?>
    <?php 
        ...
        $latestHtml = getGhoshnaHtml($latest);
    ?>
    
    <div class="featured-item">
        <div class="featured-badge">प्रमुख</div>
        <div class="featured-content">
            <?php echo nl2br(htmlspecialchars($latest['slogan_sanskrit'])); ?>
        </div>
        <p style="text-align:center; color:var(--ink-muted); margin-bottom:20px;"><?php echo htmlspecialchars($latest['slogan_hindi']); ?></p>
        <div style="text-align:center;">
            <button class="detail-link-btn" style="display:inline-block; width:auto; padding:10px 30px;" onclick="openSidePanel('घोषणा', `<?php echo addslashes($latestHtml); ?>`)">विस्तार से देखें एवं साझा करें</button>
        </div>
    </div>

    <h3 style="margin-bottom:20px; color:var(--ink-muted); font-size:1.1rem; border-bottom:1px solid var(--border-warm); padding-bottom:8px;">अन्य घोषणाएं</h3>
    <div class="archive-grid">
        <?php foreach ($items as $item): 
            $title = !empty($item['slogan_sanskrit']) ? mb_substr($item['slogan_sanskrit'], 0, 60) . '...' : 'घोषणा';
            $itemHtml = getGhoshnaHtml($item);
        ?>
            <div class="grid-item" onclick="openSidePanel('घोषणा', `<?php echo addslashes($itemHtml); ?>`)">
