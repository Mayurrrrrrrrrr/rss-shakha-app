...
function getSubhashitHtml($item) {
    ...
    $currentUrl = "https://sanghasthan.yuktaa.com/subhashit/#item-" . $item['id'];
...
<div class="public-header">
    <h1>✨ सुभाषित</h1>
    <p>संस्कृत के अमूल्य सूक्तियों का संग्रह</p>
</div>
...
<?php else: ?>
    <div class="list-container">
        <?php foreach ($items as $index => $item): 
            $title = !empty($item['sanskrit_text']) ? mb_substr($item['sanskrit_text'], 0, 80) . '...' : 'सुभाषित';
            $itemHtml = getSubhashitHtml($item);
            $isLatest = ($index === 0);
        ?>
            <div class="list-item" id="item-<?php echo $item['id']; ?>" style="<?php echo $isLatest ? 'border-left:4px solid var(--saffron); background:var(--saffron-glow);' : ''; ?>" 
