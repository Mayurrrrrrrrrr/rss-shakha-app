...
<?php else: ?>
    <div class="ghoshna-grid">
        <?php foreach ($items as $index => $item): 
            ...
            $isLatest = ($index === 0);
        ?>
            <div class="ghoshna-card" id="item-<?php echo $item['id']; ?>" <?php echo $isLatest ? 'style="border-top:4px solid var(--saffron);"' : ''; ?>>
                ...
                <div class="ghoshna-footer">
                    ...
                    <?php
                        $shareText = "🗣️ *" . ($item['slogan_sanskrit'] ?? 'घोषणा') . "*\n\n" . ($item['slogan_hindi'] ?? '') . "\n\nविस्तार से पढ़ें: https://sanghasthan.yuktaa.com/ghoshnayein/#item-" . $item['id'] . "\n\n🚩 *संघस्थान*";
                        $waLink = "https://api.whatsapp.com/send?text=" . urlencode($shareText);
                    ?>
                    <a href="<?php echo $waLink; ?>" target="_blank" style="color:#25D366; text-decoration:none; display:inline-flex; align-items:center; gap:4px; font-weight:700;">
                        <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M12.012 2c-5.508 0-9.988 4.48-9.988 9.988 0 1.76.459 3.413 1.26 4.852l-1.34 4.896 5.01-1.316c1.408.766 3.013 1.204 4.718 1.204 5.508 0 9.988-4.48 9.988-9.988s-4.48-9.988-9.988-9.988zm6.596 14.152c-.273.766-1.353 1.433-2.222 1.543-.591.074-1.363.132-3.961-.933-3.322-1.363-5.464-4.738-5.63-4.956-.165-.219-1.339-1.782-1.339-3.411 0-1.63.847-2.43 1.15-2.76.303-.33.666-.412.889-.412.222 0 .444.004.639.013.199.008.468-.076.734.568.273.659.932 2.274 1.012 2.438.079.164.133.356.024.573-.109.219-.164.356-.328.547-.164.192-.345.426-.492.573-.165.164-.338.344-.145.679.193.336.858 1.413 1.838 2.285.98 0.872 1.808 1.144 2.138 1.309.33.164.52.14.714-.079.192-.219.824-.961 1.042-1.285.219-.328.437-.273.738-.164.301.109 1.913.902 2.24 1.066.328.164.547.245.628.383.082.138.082.802-.191 1.568z"/></svg> शेयर
                    </a>
                </div>
            </div>
        <?php endforeach; ?>
